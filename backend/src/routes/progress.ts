...
import { getProgress, getResumePosition, recordProgress, overrideProgress } from '../services/progressStore';
...
router.post('/:jobId/override', (req, res) => {
  const { jobId } = req.params;
  const { current } = req.body ?? {};

  if (typeof current !== 'number' || !Number.isFinite(current)) {
    res.status(400).json({ error: 'current must be provided as a finite number.' });
    return;
  }

  const entry = overrideProgress(jobId, current);
  if (!entry) {
    res.status(404).json({ error: 'Progress not found for job.' });
    return;
  }
